	...
	showOpenedDealsList() {
		...
		const readyList = active_deals.map((item, key) => {
			return (
				...
				<div
					key={key}
					className={`deals__item btn deal opened`}
				>

					<div className="item__header layer head">
						<div className="item__name row one">
							<CurrentChartInfo data={item}/>
						</div>
					</div>
					<div className="item__body row three status">

						<div className="deals__item-col item__way">
							<div className="amount__icon">
								<div className={`fa fa-arrow-${item.type === 'up' ? 'up' : 'down'} arrow`}/>
							</div>
							<div className={`${item.type === 'up' ? 'amount__value-up' : 'amount__value-down'} amount__value`}>{item.amount.toFixed(2)}  $</div>
						</div>
						<div className="deals__item-col item__timer">
	...
	showClosedDealsList() {
		...
		const readyList = closed_deals.map((item, key) => {
			...
			return (
				...
				<div
					key={key}
					className={`deals__item btn deal ${selected ? 'selected' : ''}`}
				...
				>
					...
					<div className="item__body row three status">
						...
						<div className="deals__item-col item__cost">
							<div className={item.status}>{income} $</div>
						</div>
						<div className="deals__item-col item__icon">
							<img className={`item__icon-${selected ? 'down' : 'up'} arrow`} src="icons/arrow_deal_thin.svg" alt=""/>
						</div>
					</div>
	...
	render() {
		...
		return (
			<div className="slide-deals">
				...
				<div className="deals_box__header dealsBoxHeader">
					<div className="header__btn header__btn-opened">
						<img src="icons/arrow_deal.svg" alt=""/>
					</div>
					<div className="header__title">Сделки</div>
				</div>
				<Tabs
