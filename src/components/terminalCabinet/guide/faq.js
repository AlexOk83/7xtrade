...
import { historyIncreases } from '../../../actions/payment'

class HistoryIncreases extends React.Component {
	...
	render() {
		const { 
			...
			count,
			isLoaded
		} = this.props.payment.history_increases;
		...
		return (
			<div className="history-increases">
				<CabinetTable 
					...
					requestToServer={params => this.props.historyIncreases(params)}
				/>
			</div>
		)
	}
...
HistoryIncreases.propTypes = {
	historyIncreases: PropTypes.func.isRequired,
...
export default connect(mapStateToProps, mapDispatchToProps)(HistoryIncreases);